...
import { readFreshness } from './freshness';
...
export function PriceFreshness({ now = new Date() }: { now?: Date }) {
  ...
  const { ageDays, stale } = readFreshness(data.pricesCapturedAt, now);
    ...
    <div className="text-ink-faint space-y-1 text-xs">
      ...
      {stale ? (
        <p role="status" className="text-promo flex items-center gap-1.5">
          ...
          <span>
            Prijsgegevens zijn mogelijk verouderd — ze zijn {ageDays} dagen oud. De bedragen kloppen
