...
sections.push(
  insert(
    ...
    [
      'id', 'canonical_name', 'category', 'default_base_unit', 'density',
      'piece_weight_grams', 'perishability', 'vegetarian', 'vegan',
      'pantry_staple', 'allergens', 'pregnancy_risks',
    ],
    SEED_INGREDIENTS.map((i) => [
      i.id, i.canonicalName, i.category, i.baseUnit, i.density ?? null,
      i.pieceWeightGrams ?? null, i.perishability, i.vegetarian, i.vegan,
      i.pantryStaple === true, i.allergens, i.pregnancyRisks,
    ]),
  ),
);
...
sections.push(
  insert(
    ...
    [
      'ingredient_id', 'kcal_per_100', 'protein_per_100', 'carbs_per_100',
      'sugars_per_100', 'fat_per_100', 'saturated_per_100', 'fiber_per_100',
      'salt_per_100', 'micronutrients', 'source',
    ],
    SEED_INGREDIENTS.filter((i) => i.nutritionPer100).map((i) => {
      const n = i.nutritionPer100!;
      return [
        i.id, n.kcal, n.protein, n.carbohydrates, n.sugars, n.fat,
        n.saturatedFat, n.fiber, n.salt, n.micronutrients ?? {},
...
sections.push(
  insert(
    'recipes',
    [
      'id', 'name', 'description', 'image_url', 'steps', 'prep_minutes',
      'cook_minutes', 'difficulty', 'cuisine', 'tags', 'base_servings',
      'primary_protein', 'authored_kcal', 'authored_protein', 'authored_carbs',
      'authored_fat', 'authored_fiber', 'authored_salt',
    ...
    recipes.map((r) => {
      ...
      return [
        r.id, r.name, r.description, r.imageUrl, r.steps, r.prepMinutes,
        r.cookMinutes, r.difficulty, r.cuisine, r.tags, r.baseServings,
        r.primaryProtein, authored.kcal, authored.proteinGrams, authored.carbGrams,
        authored.fatGrams, authored.fiberGrams, authored.saltGrams,
        override ?? null,
      ];
    }),
  ),
...
sections.push(
  insert(
    ...
    ['id', 'chain_id', 'name', 'address', 'postal_code', 'city', 'latitude', 'longitude', 'region_id'],
    SEED_LOCATIONS.map((l) => [
      l.id, l.chainId, l.name, l.address, l.postalCode, l.city, l.latitude, l.longitude, l.regionId,
    ]),
  ),
);
...
sections.push(
  insert(
    'products',
    [
      'id', 'gtin', 'brand_id', 'product_name', 'canonical_ingredient_id',
      'package_amount', 'package_unit', 'chain_id', 'active', 'image_url',
    ],
    products.map((p) => [
      p.id, p.gtin ?? null, p.brandId, p.productName, p.canonicalIngredientId,
      Number(p.packageAmount.amount.toFixed(3)), p.packageAmount.unit, p.chainId,
      p.active, p.imageUrl ?? null,
    ]),
  ),
);
...
sections.push(
  insert(
    'product_nutrition',
    [
      'product_id', 'kcal_per_100', 'protein_per_100', 'carbs_per_100',
      'sugars_per_100', 'fat_per_100', 'saturated_per_100', 'fiber_per_100',
      'salt_per_100', 'micronutrients', 'source',
    ],
    productNutrition.map((entry) => [
      entry.productId, entry.per100.kcal, entry.per100.protein, entry.per100.carbohydrates,
      entry.per100.sugars, entry.per100.fat, entry.per100.saturatedFat, entry.per100.fiber,
      entry.per100.salt, entry.per100.micronutrients ?? {}, entry.source,
    ]),
  ),
);
...
sections.push(
  insert(
    'product_prices',
    [
      'product_id', 'chain_id', 'location_id', 'region_id', 'scope',
      'price_cents', 'valid_from', 'valid_until', 'observed_at', 'source',
    ],
    observations.map((o) => [
      o.productId,
...
sections.push(
  insert(
    ...
    [
      'product_id', 'chain_id', 'location_id', 'region_id', 'scope',
      'promotion_type', 'unit_price_cents', 'percent', 'bundle_size',
      'bundle_price_cents', 'minimum_quantity', 'label', 'valid_from',
      'valid_until', 'source',
    ],
    promotions.map((p) => {
